...
def breadthFirstSearch(problem: SearchProblem):
    ...
    util.raiseNotDefined()
def bestFirstSearch(problem: SearchProblem, heuristic):
    """Search the node that has the lowest heuristic first."""
    "*** YOUR CODE HERE ***"
    my_prioqueue = util.PriorityQueue()
    init_state = problem.getStartState()
    init_superstate = Superstate(init_state, None, None, heuristic(init_state, problem))
    visited_list = [init_superstate]
    my_prioqueue.push(init_superstate, heuristic(init_state, problem))
    ...
    while not my_prioqueue.isEmpty():
        ...
        if problem.isGoalState(current_superstate.state):
            ...
            successors = problem.getSuccessors(current_superstate.state)
            for successor in list(successors):
                successors_superstate = Superstate(successor[0], current_superstate, successor[1], heuristic(successor[0], problem))
                for i, each in enumerate(visited_list):
                    if successors_superstate.state == visited_list[i].state:
                        break
                    if i == len(visited_list) - 1:
                        visited_list += [successors_superstate]
                        my_prioqueue.push(successors_superstate, heuristic(successor[0], problem))
