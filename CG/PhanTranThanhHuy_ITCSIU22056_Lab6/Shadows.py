...
class Example(Base):
    ...
    Scene with:
    - Textured sky
    - Grass that receives shadows
    - Sun sphere that is pure yellow (not shaded by lighting)
    - A shaded Earth sphere on the ground lit by the Sun and casting a shadow
    """
    def initialize(self):
        ...
        # --- Grass (Phong so it can receive shadows) ---
        ...
        # --- Earth sphere (shaded by lights + casts shadow) ---
    ...
    def update(self):
        ...
        sun_x = 6 * math.cos(0.8 * self.time)
        sun_y = 8
        sun_z = 6 * math.sin(0.8 * self.time)
