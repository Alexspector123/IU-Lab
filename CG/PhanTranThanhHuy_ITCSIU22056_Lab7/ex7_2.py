...
# ---------------------------------------------------------
# Wrapper: Phong that just cleans unsupported props
# ---------------------------------------------------------
class SurfaceMaterial(PhongMaterial):
    def __init__(self, *args, **kwargs):
        # Only clean property_dict; let py3d manage light count/shadows
...
class Example(Base):
    def __init__(self, *args, detail: float = 1.0, **kwargs):
        """
        detail:
            1.0 = full city (5x5 roads, dense buildings/trees/dashes)
            0.5 = medium city (3x3 roads, medium density)
            0.3 ~ 0.4 = light city (compact, fewer objects)
        """
        self.detail = detail
        super().__init__(*args, **kwargs)
    ...
    def make_tree(self, x, z, small=False):
        """
        Use precreated small/big tree geometries, no scaling API needed.
        """
        if small:
            trunk_geom = self.tree_trunk_geom_small
            crown_geom = self.tree_crown_geom_small
            trunk_h = 0.9
            crown_r = 0.6
        else:
            trunk_geom = self.tree_trunk_geom_big
            crown_geom = self.tree_crown_geom_big
            trunk_h = 1.2
            crown_r = 0.8

        trunk = Mesh(trunk_geom, self.tree_trunk_mat)
        trunk.set_position([x, trunk_h / 2, z])
        self.scene.add(trunk)

        crown = Mesh(crown_geom, self.tree_crown_mat)
    ...
    def init_pedestrians(self):
        ...
        # On very low detail, skip pedestrians entirely for performance
        if self.detail < 0.4:
            return
    ...
    def initialize(self):
        ...
        # Lighting: single directional light (simple & consistent)
        ...
        self.traffic_ns_green_time = 7.0  # first half: NS green, EW red
        ...
        # Road centers depend on detail level
        if self.detail >= 0.9:
            coords = [-60, -30, 0, 30, 60]   # 5x5 grid (full city)
        elif self.detail >= 0.5:
            coords = [-40, 0, 40]            # 3x3 grid (medium city)
        else:
            coords = [-30, 0, 30]            # compact 3x3 grid
        ...
        # dash density depends on detail
        dash_step = 8 if self.detail >= 0.9 else (16 if self.detail >= 0.5 else 24)

        # along every east–west road
        for z in self.road_z:
            for x in range(-90, 91, dash_step):
        ...
        for x in self.road_x:
            for z in range(-90, 91, dash_step):
        ...
        # ========= Shared tree geometry/material (small & big) =========
        self.tree_trunk_geom_big = BoxGeometry(0.25, 1.2, 0.25)
        self.tree_trunk_geom_small = BoxGeometry(0.25, 0.9, 0.25)
        self.tree_crown_geom_big = SphereGeometry(0.8)
        self.tree_crown_geom_small = SphereGeometry(0.6)
        self.tree_trunk_mat = SurfaceMaterial(
            property_dict={"baseColor": [0.45, 0.28, 0.15]}
        )
        self.tree_crown_mat = SurfaceMaterial(
            property_dict={"baseColor": [0.15, 0.65, 0.15]}
        )
        ...
        for i in range(len(self.road_x) - 1):
            ...
            for j in range(len(self.road_z) - 1):
                ...
                # At low detail, sometimes skip entire block
                if self.detail < 0.5 and random.random() < 0.4:
                    continue
                ...
                for gx in range(grid_nx):
                    for gz in range(grid_nz):
                        ...
                        # leave some empty cells (more emptiness for low detail)
                        if self.detail >= 0.9:
                            empty_prob = 0.12
                        elif self.detail >= 0.5:
                            empty_prob = 0.25
        ...
        # density: bigger step on lower detail
        tree_step = 10 if self.detail >= 0.9 else (20 if self.detail >= 0.5 else 30)

        # along E–W roads
        for z in self.road_z:
            for x in range(-80, 81, tree_step):
                for sign in (+1, -1):
                    tx = x
        ...
        # along N–S roads
        for x in self.road_x:
            for z in range(-80, 81, tree_step):
                for sign in (+1, -1):
    ...
    def update(self):
        ...
        # Render main camera (mini-map possible with sky_camera)
...
if __name__ == "__main__":
    # Recommended for students (weaker laptops)
    Example(screen_size=[640, 480], detail=0.5).run()
    # On stronger machines you can try:
    # Example(screen_size=[800, 600], detail=1.0).run()
