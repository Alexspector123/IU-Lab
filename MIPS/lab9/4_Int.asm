...
printSortedArray: .asciiz "The sorted array (Before normalize): "
printNormalizeArray: .asciiz "The sorted array (After normalize): "
strLine: .asciiz "\n"
strSpace: .asciiz " "
...
main:
	...
	la $a0, printNormalizeArray
	li $v0, 4
	syscall

	mtc1 $t3, $f0			# Convert the maximum element $t3 from int to float $f0
	cvt.s.w $f0, $f0
	la $a1, arr			# $a1 = address of arr
	jal NORMALIZE			# Display the normalized array
...
MAXIMUM:
	...
	sub $s0, $s0, 1			# $s0 = $s0 - 1
	sll $s0, $s0, 2			# $s0 = $s0 * 4
	add $t0, $t0, $s0		# arr = arr + $s0

	lw $t3, 0($t0)			# $t3 = arr[$s0]
...
NORMALIZE:
	...
	mtc1 $t2, $f2			# Convert $t2 from int to float $f2
	cvt.s.w $f2, $f2

	div.s $f2, $f2, $f0		# Divide $f2 = $f2/$f0 = arr[i]/Maximum element

	mov.s $f12, $f2			# Print the $f2
	li $v0, 2
	syscall
